angular.module('listings').controller('ListingsController', ['$scope', '$location', '$stateParams', '$state', 'Listings', 
  function($scope, $location, $stateParams, $state, Listings){
    ...
    $scope.update = function(isValid) {
      ...
      Listings.update($stateParams.listingId, values).then(function(response) {
          $state.go('listings.list', {successMessage: 'Listing updated'});
        }, function(error){
          $scope.error = 'Cannot update listing\n' + error;
        });
    };
    ...
    $scope.remove = function() {
      ...
      Listings.delete($stateParams.listingId).then(function(repsonse) {
          $state.go('listings.list', {successMessage: 'Listing deleted'});
      }, function(error) {
        $scope.error = 'Cannot delete listing\n' + error;
      });
    };
